...
public final class PermsEvents {
    ...
    public static void onGatherNodes(PermissionGatherEvent.Nodes event) {
        ...
        adminNode = new PermissionNode<>("panoptic", "admin", PermissionTypes.BOOLEAN,
                (player, uuid, context) -> player != null && player.hasPermissions(3));
    ...
    public static void sync(ServerPlayer p) {
        ...
        Panopticon.CHANNEL.send(PacketDistributor.PLAYER.with(() -> p), new PermsSyncPacket(nodes));
        ...
        Panopticon.CHANNEL.send(PacketDistributor.PLAYER.with(() -> p),
                new SeedPushPacket(seedGranted, seedGranted ? seed : 0L));
    }
    ...
    public static void onServerTick(TickEvent.ServerTickEvent event) {
        if (Panopticon.CHANNEL == null || event.phase != TickEvent.Phase.END
                || ++tick % 40 != 0) {
            return;
        }
